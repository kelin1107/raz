...
SUBTOPICS = [
    # ---------------- 生命世界 ----------------
    ('生命世界', 'bird', r'\bbird\b|birds|eagle|owl|penguin|hen|chicken|duck|goose|robin|parrot|flamingo', '🐦 鸟类',
    ...
    ('生命世界', 'sea', r'fish|shark|dolphin|whale|seal|octopus|jellyfish|crab|lobster|sea|ocean|underwater|turtle|fish', '🐠 海洋动物',
    ...
    ('生命世界', 'insect', r'insect|bug|butterfly|bee|ant|spider|worm|beetle|dragonfly|ladybug', '🐛 昆虫与虫',
    ...
    ('生命世界', 'plant', r'plant|flower|tree|seed|leaf|leaves|garden|grass|fruit|vegetable|root|stem|forest', '🌱 植物',
    ...
    ('生命世界', 'sense', r'sense|senses|eye|ear|nose|tongue|skin|smell|taste|touch|hear|see', '👀 感官',
    ...
    ('生命世界', 'mammal', r'otter|\bbats?\b', '🐾 哺乳动物',
     '这几本都讲哺乳动物（如河狸/蝙蝠）：外形、皮毛、住哪、吃什么、怎么带宝宝。一天读，fur/body/baby 反复出现，孩子把「胎生、吃奶、有毛」这件事想明白，并练习用同一组词描述不同的哺乳动物。',
    ...
    ('地球与宇宙', 'earth', r'earth|rock|soil|mountain|volcano|land|ground|erosion|earthquake|mineral|sand|fossi', '⛰️ 岩石与土壤',
     '岩石土壤书讲地球表面。一天读，rock/soil/mountain 反复出现，孩子理解「脚下的地」从哪来。',
     '1) 石头盒：收集不同石头/土分类。\n2) 火山：小苏打+醋模拟 eruption。\n3) 画：画一层层地层。', 'opt',
     '剖面图：草地→土壤层→岩石层，小铲子挖出石头，土系水彩。'),
    ('地球与宇宙', 'space', r'sun|moon|earth|space|planet|solar|star|universe|orbit|night|day|astronaut|rocket|galaxy', '🪐 太空与日月',
     '太空书讲太阳/月亮/行星/白天黑夜。一天读，sun/moon/earth/star 反复出现，孩子把「天为啥会亮」讲明白。',
     '1) 模拟：球+灯演 地球转出自天黑。\n2) 贴纸：贴出太阳系顺序。\n3) 画：画白天/黑夜各做什么。', 'strong',
     '夜空横向：太阳/月亮/地球/星星+小火箭，深蓝点缀亮星，梦幻水彩。'),

    # ---------------- 物质与能量 ----------------
    ('物质与能量', 'light', r'light|shadow|reflect|mirror|sunlight|dark|bright|rainbow|color', '💡 光与影子',
    ...
    ('物质与能量', 'heat', r'heat|hot|cold|temperature|warm|cool|thermometer|burn|fire|mel', '🌡️ 冷与热',
    ...
    ('物质与能量', 'matter', r'solid|liquid|gas|state|matter|mixture|property|material|float|sink', '🧪 物质三态',
     '物质书讲固/液/气。一天读，solid/liquid/gas/water 反复出现，孩子把「同一样东西三种样子」分清。',
     '1) 三态：找冰/水/蒸汽对应 solid/liquid/gas。\n2) 融化：冰→水观察。\n3) 画：画三态。', 'opt',
     '三态对比：冰块/水杯/云气同框，蓝白水彩。'),

    # ---------------- 身体与健康 ----------------
    ('身体与健康', 'body', r'body|heart|lung|bone|bones|muscle|muscles|blood|brain|stomach|digest|organ|skeleton', '🫀 身体器官',
    ...
    ('思维与创意', 'color', r'color|red|blue|yellow|green|brown|black|white|mix|paint|primary', '🎨 颜色',
     '颜色书讲红黄蓝与混色。一天读，red/blue/yellow 反复出现，孩子把「混出橙绿紫」试出来。',
     '1) 调色：红+黄=橙 做实。\n2) 找色：家里找三原色物。\n3) 画：画色轮。', 'strong',
     '调色盘图：红黄蓝三原色+混出橙绿紫，溅色水彩。'),
    ('思维与创意', 'size', r'size|big|small|long|short|tall|heavy|light|wide|narrow|thick|thin|measure', '📏 大小',
     '大小书讲大/小/长/短。一天读，big/small/long 反复出现，孩子练「排一排」。',
     '1) 排队：把玩具按大小排。\n2) 比：用身体比 tall/short。\n3) 画：画大小对比。', 'opt',
     '大小阶梯：动物/杯子由小到大排列，渐变水彩。'),
    ('思维与创意', 'opposite', r'opposite|opposites|concept|same|different|compare|pair|up|down|in|out|fast|slow', '🔁 反义词',
     '反义词书讲 大/小、上/下、快/慢。一天读，一对对词同时出现，孩子一次认两组相对概念。',
     '1) 配对卡：画反义词卡玩配对。\n2) 动作：用身体演 up/down。\n3) 画：画一对反义词。', 'opt',
     '反义词对对碰：大/小、上/下、快/慢各一格，对比萌趣。'),
    ('思维与创意', 'art', r'art|draw|paint|music|dance|craft|sing|song|instrument|creative|make', '🎶 艺术与创作',
